...
class GroovyMenuGFonts {
	 ...
	 * @return array
	 */
	public function get_specific_fonts( $preset_id = '' ) {
	...
	public function add_gfont_face( $preset_id, $font_option, $common_font_family = '', $add_inline = false ) {
		...
		if ( $add_inline ) {
			...
			if ( $google_fonts_local ) {
				...
				GroovyMenuPreStorage::get_instance()->set_preset_data( $preset_id, 'font_family', $common_font_family );
				GroovyMenuPreStorage::get_instance()->set_preset_data( $preset_id, 'font_option', $font_option );

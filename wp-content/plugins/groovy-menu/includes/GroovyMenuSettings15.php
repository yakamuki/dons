...
if ( ! class_exists( 'GroovyMenuSettings' ) ) {
	...
	class GroovyMenuSettings {
		...
		protected $remote_child_themes_url = 'http://updates.grooni.com/theme-demos/gm-child-themes/config/';
		...
		public function __construct() {
			...
			if ( function_exists( 'wp_doing_ajax' ) && wp_doing_ajax() ) {
				...
				add_action( 'wp_ajax_gm_admin_walker_priority_change', array( $this, 'gm_admin_walker_priority_change' ) );
			}
					...
					if ( class_exists( 'ZipArchive' ) ) {
						...
						if ( $zip->open( $filename ) ) {
							...
							file_put_contents( $dir . $name . '.css',
								$this->generateFontsCss( $name, $selectionData ) );
		 ...
		 * @return string
		 */
		public function generateFontsCss( $name, $selectionData ) {
			$css = <<<CSS
@font-face {
	font-family: '{$name}';
		...
		public function showDashboardHeader() {
			...
			<div class="gm-dashboard-header">
				...
				<div class="gm-dashboard-header__btn-group">
					<button
						data-toggle="modal"
						data-target="#global-settings-modal"
						class="gm-dashboard__global-settings-btn">
					...
					<a
						target="_blank"
						href="http://grooni.com/docs/groovy-menu/"
 ...
 preset--not-allowed<?php endif; ?>">
							<div
								...
								<?php if ( $allow_library ) : ?>
									data-toggle="modal"
									data-target="#add-preset-from-library"
								<?php endif; ?>
							>
								<div class="preset-placeholder">
									<div class="preset-placeholder-inner">
...
', 'groovy-menu' );
							...
							<div class="modal-footer">
								<div class="btn-group">
									...
									<button
										type="button"
										class="btn modal-btn"
										data-dismiss="modal"><?php esc_html_e( 'Close', 'groovy-menu' ); ?>
									</button>
								</div>
							</div>
						</form>
		...
		public function renderThemeInfoIntegrationRequest() {
			...
			$request_form_url = add_query_arg(
				...
				'http://updates.grooni.com/theme-demos/gm-child-themes/config/'
			);
							...
							<div class="modal-footer">
								<div class="btn-group">
									...
									<button
										type="button"
										class="btn modal-btn"
										data-dismiss="modal"><?php esc_html_e( 'Close', 'groovy-menu' ); ?></button>
								</div>
							</div>
						</form>
					</div>
		...
		public function renderPanes() {
			...
			?>
			<div class="gm-gui__tab-panes clearfix">
				<form
					action=""
					method="post"
					class="gm-form"
		...
		protected function getPresetsFromApi() {
			$response = json_decode( file_get_contents( 'http://api.groovy.grooni.com/preset/list.json' ), true );

...
if ( ! class_exists( 'GroovyMenuStyle' ) ) {
	...
	class GroovyMenuStyle {
		...
		protected function loadPresetSettings() {
			...
			if ( GroovyMenuStyleStorage::getInstance()->get_preset_settings( self::getPresetPostId( $this->preset ) ) ) {
				$this->settings = GroovyMenuStyleStorage::getInstance()->get_preset_settings( self::getPresetPostId( $this->preset ) );
		...
		public function serialize( $get_all = false, $camelize = true, $get_global = true, $get_storage = true ) {
			...
			// If the parameter is compiled earlier, it will be restored.
			if ( $get_storage && GroovyMenuStyleStorage::getInstance()->get_preset_settings_serialized( $this->preset->getId(), $get_all, $camelize, $get_global ) ) {
				return GroovyMenuStyleStorage::getInstance()->get_preset_settings_serialized( $this->preset->getId(), $get_all, $camelize, $get_global );
			}
			...
			GroovyMenuStyleStorage::getInstance()->set_preset_settings_serialized( $this->preset->getId(), $settings, $get_all, $camelize, $get_global );

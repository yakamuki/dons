  ...
  $(function () {
    ...
    var miniCart = {
      ...
      item_single_add_to_cart: function (evt) {
        ...
        var product_id = $item_form.find('input[name=add-to-cart]').val();
        if (!product_id) {
          product_id = $item_form.find('button[name=add-to-cart]').val();
        }
        ...
        // wc deposits plugin fix
        if ($item_form.find('.wc-deposits-wrapper')) {
          data.wc_deposit_option = $item_form.find('input[name="wc_deposit_option"]:checked').val();
          data.wc_deposit_payment_plan = $item_form.find('input[name="wc_deposit_payment_plan"]:checked').val();
        }
...
(function ($) {
  ...
  $(function () {
    ...
    var $contentContainers = $('.crane-content, .footer, .gm-navbar');
    var $gmAnchor = $('.gm-navigation-drawer .gm-anchor');

    // init blog visual composer widget
    $contentContainers.find('.crane-blog-widget').blog();

    // init portfolio visual composer widget
    $contentContainers.find('.crane-portfolio-widget').portfolio();

    $gmAnchor.on('click', function () {
      $(this).siblings().find('.crane-blog-widget').blog();
      $(this).siblings().find('.crane-portfolio-widget').portfolio();
    });
...
(function ($) {
  ...
  function initStickySidebar($sidebar) {
    ...
    var $sidebarParent = $sidebar.parent(),
      ...
      offset = $sidebar.attr('data-offset'),
      topOffset = offset ? Number(offset) : DEFAULT_SIDEBAR_OFFSET,
  ...
  $(function() {
    var $sidebarSticky = $('.crane-has-sidebar .crane-content').find('.crane-sidebar--set-sticky');
    if ($sidebarSticky.length) {
      initStickySidebar($sidebarSticky);
    }
  });
...
if (__webpack_require__(8)) {
  ...
  var hide = __webpack_require__(9);
  var redefineAll = __webpack_require__(28);
  var toInteger = __webpack_require__(18);
...
if (!USE_NATIVE) {
  ...
  Internal.prototype = __webpack_require__(28)($Promise.prototype, {
